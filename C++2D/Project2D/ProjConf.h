...
#define ENE2_POS_Y_INI	(STG_Y_MAX - 8 * CELL)	// 敵2初期位置Y（下から8ブロック目）
#define ENE3_POS_X_INI	240						// 敵3初期位置X
#define ENE3_POS_Y_INI	(STG_Y_MAX - 13 * CELL)	// 敵3初期位置Y（下から13ブロック目）
...
#define MOVEX		(CELL / 8)					// X方向移動量（通常時）
#define MOVEX_D		(CELL / 4)					// X方向移動量（ダッシュ時）
#define MOVEY_H_MAX	(4.5 * CELL)					// ジャンプ時の最大高さ（5ブロック目）

#define GOAL_POS_X (5400 - WIN_MAX_X / 2)		// GOAL X位置

/*** クリア設定 ***/
#define CLE_ENTER_POS_X 220		// 'Press Enter' X位置
#define CLE_ENTER_POS_Y 400		// 'Press Enter' Y位置
